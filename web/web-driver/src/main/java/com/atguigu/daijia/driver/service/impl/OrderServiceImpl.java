...
// 补充：订单数判空（避免传null给规则服务）
        ...
        // 核心修复：rewardRuleResponseVo为空时初始化空对象，避免copyProperties传null
        if (rewardRuleResponseVo == null) {
            log.warn("结束代驾：系统奖励规则计算结果为空，使用默认值，orderId={}", orderFeeForm.getOrderId());
            rewardRuleResponseVo = new RewardRuleResponseVo(); // 初始化空对象（所有字段默认值）
        }
